...
router.get('/', requireAdmin, async (req, res) => {
  ...
  const { data: allRooms } = await supabase.from('rooms').select('*').eq('hotel_id', hotelId).order('number');
  ...
  function renderReservation(r) {
    ...
    return '<div class="res-card" style="' + (r.booking_group_id ? 'border-left:3px solid #534AB7;' : '') + '">' +
      ...
      '</div>' +
      '<div class="res-type">🛏 ' + (r.room_types?.name || '—') +
        (r.rooms?.number ? ' · <strong>Room ' + r.rooms.number + '</strong>' : ' · <span style="color:#b07a00;">No room assigned yet</span>') +
      '</div>' +
      (r.notes ? '<div class="res-notes">📝 ' + r.notes + '</div>' : '') +
      '<div class="res-actions">' +
        (!isCheckedIn ? '<form method="POST" action="/reservations/' + r.id + '/checkin-name" style="flex:1;"><input type="hidden" name="guest_name" value="' + r.guest_name + '"/><button class="btn btn-success" type="submit" onclick="return promptName(this)">Check in</button></form>' : '') +
        '<form method="POST" action="/reservations/' + r.id + '/checkout" onsubmit="return confirm(\'Check out ' + r.guest_name + '?\')" style="flex:1;"><button class="btn btn-ghost" type="submit">Check out</button></form>' +
      '</div>' +
    '</div>';
  }
  ...
  Object.entries(groups).forEach(([groupId, items]) => {
    ...
    resCards += '<div style="margin-bottom:16px;"><div style="font-size:13px;font-weight:700;color:#534AB7;margin-bottom:8px;">👥 Group booking: ' + bookerName + ' · ' + items.length + ' rooms</div>' + items.map(renderReservation).join('') + '</div>';
  });
  resCards += ungrouped.map(renderReservation).join('');

  const roomTypeOptions = (roomTypes || []).map(rt => '<option value="' + rt.id + '">' + rt.name + '</option>').join('');
  ...
  const pageBody = '<div class="page-title">Reservations</div>' +
    ...
    '</form>' +
    ((reservations || []).length === 0 ? '<div class="empty"><div class="empty-icon">📋</div><div class="empty-text">No active reservations</div></div>' : resCards) +
    '<div class="card" style="margin-top:24px;">' +
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">' +
        ...
        '<label style="display:flex;align-items:center;gap:6px;font-size:13px;color:#534AB7;font-weight:600;cursor:pointer;">' +
          '<input type="checkbox" id="multi-room-toggle" onchange="toggleMultiRoom()"/> Multiple rooms' +
        '</label>' +
      '</div>' +
      '<form method="POST" action="/reservations/add" id="reservation-form">' +
        '<div class="form-group"><label class="form-label" id="name-label">Guest Name</label>' +
        '<input class="form-input" type="text" name="guest_name" id="booker-name" placeholder="Full name" required/></div>' +
        '<div class="form-group"><label class="form-label">Phone Number</label>' +
        '<input class="form-input" type="tel" name="guest_phone" placeholder="+389 ..."/></div>' +
        '<div id="single-room-group">' +
          '<div class="form-group"><label class="form-label">Room Type</label>' +
          '<select class="form-input" name="room_type_id" id="single-room-type" onchange="updateRoomPicker(this.value, \'single-room-picker\')">' +
          '<option value="">Select type</option>' + roomTypeOptions + '</select></div>' +
          '<div class="form-group"><label class="form-label">Specific Room (optional)</label>' +
          '<select class="form-input" name="room_id" id="single-room-picker">' +
          '<option value="">Any room of this type (assign later)</option></select></div>' +
        '</div>' +
        '<div class="form-group" id="multi-room-group" style="display:none;">' +
          '<label class="form-label">Rooms Needed</label>' +
          '<div id="multi-room-list" style="display:flex;flex-direction:column;gap:10px;"></div>' +
        ...
        '</div>' +
        '<div class="form-row">' +
          '<div class="form-group"><label class="form-label">Check In</label><input class="form-input" type="date" name="check_in" value="' + today + '" required/></div>' +
          '<div class="form-group"><label class="form-label">Check Out</label><input class="form-input" type="date" name="check_out" required/></div>' +
        '</div>' +
        '<div class="form-group"><label class="form-label">Notes (optional)</label><input class="form-input" type="text" name="notes" placeholder="Early check-in, extra bed..."/></div>' +
        '<button class="btn btn-primary" type="submit">Save Reservation</button>' +
      '</form>' +
    '</div>' +
    '<script>' +
      'const roomsByType = ' + JSON.stringify(roomsByType) + ';' +
      'const roomTypeOptionsHTML = \'<option value="">Select type</option>' + roomTypeOptions.replace(/'/g, "\\'") + '\';' +
      'let roomRowCount = 0;' +
      'function updateRoomPicker(typeId, pickerId) {' +
      ...
      'function toggleMultiRoom() {' +
        ...
        'if (isMulti && roomRowCount === 0) { addRoomRow(); addRoomRow(); }' +
      '}' +
      'function addRoomRow() {' +
        'roomRowCount++;' +
        'const rowId = "multi-picker-" + roomRowCount;' +
        'const div = document.createElement("div");' +
        'div.style.cssText = "display:flex;flex-direction:column;gap:6px;background:#f8f8fa;padding:10px;border-radius:10px;";' +
        'div.innerHTML = \'<div style="display:flex;gap:8px;align-items:center;">\' +' +
          '\'<select class="form-input" name="multi_room_types[]" required style="flex:1;" onchange="updateRoomPicker(this.value, \\\'\' + rowId + \'\\\')">\' + roomTypeOptionsHTML + \'</select>\' +' +
          '\'<button type="button" onclick="this.closest(\\\'div\\\').parentElement.remove()" style="background:#fee2e2;color:#dc2626;border:none;width:36px;height:42px;border-radius:8px;cursor:pointer;font-weight:700;flex-shrink:0;">×</button>\' +' +
        '\'</div>\' +' +
        '\'<select class="form-input" name="multi_room_ids[]" id="\' + rowId + \'" style="font-size:13px;">\' +' +
  ...
  res.send(layout({ title: 'Reservations', user: req.user, body: pageBody }));
});

router.post('/add', requireAdmin, async (req, res) => {
  const hotelId = req.hotelId;
  const { guest_name, guest_phone, check_in, check_out, room_type_id, room_id, notes, multi_room_types, multi_room_ids } = req.body;

  const roomTypes = Array.isArray(multi_room_types) ? multi_room_types : (multi_room_types ? [multi_room_types] : []);
